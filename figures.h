...
GLuint _textureId[7]; //The id's of the textures for cubes
void initrand(); // random generator initialization
int randint(int min, int max); //generates a psuedo-random integer between min and max 
...
{	
	if (min>max)
    {
		return (int)(max + min*(rand()/((float)RAND_MAX + 1)));
    }
    else
    { 
		return (int)(min + max*(rand()/((float)RAND_MAX + 1)));
    }
} 
...
class Figure {
	...
	public:
		...
		Figure(float x1, float y1, float z1){
...
void Figure::rotate(){
	...
	rot_angle += 90.0; // Add 90.0 to the current position angle of figure
	
	if(rot_angle>=360.0) rot_angle=0.0;

}

// Returns current figure to the UP position
void Figure::place(){ 
...
char Figure::generate_type(){
	...
	switch(random_integer){
		...
		default:
			return 'E'; //Error: Something went wrong!  TO-DO!
			break;
	}
}
